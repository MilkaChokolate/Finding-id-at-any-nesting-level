...
/*testFunction =(arr, id) => {
     for (let i=0; i<arr.length; i++){
         function find(arr[i]){
             if (arr[i] === null || typeof(arr[i]) !== 'object' || 'isActive' in arr[i]) return arr[i];
             const temp = arr[i].constructor();

             for (let key in arr[i]) {
                 if (Object.prototype.hasOwnProperty.call(arr[i], key)) {
                     arr[i]['isActive'] = null;
                     temp[key] = key === 'id' ? arr[i] : find(arr[i][key]);
...
console.log(testFunction(home, 'id": "TEST_HOME4'));*/
let result = []
const testFunction = (data, id) => {
    const mapFunc = (arr) => {
        arr.map(item => {
            if(item.id === id){
                result.push(item)
...
console.log(testFunction(home, 'FECB1CA0-68DE-5F2C-BF3C-9070600BF8E8'))